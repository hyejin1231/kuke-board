...
public class MessageRelayCoordinator {
    ...
    public void ping() {
        redisTemplate.executePipelined((RedisCallback<?>) action -> {
            ...
            conn.zRemRangeByScore(key, Double.NEGATIVE_INFINITY, Instant.now().minusSeconds(PING_INTERVAL_SECONDS * PING_FAILURE_THRESHOLD).toEpochMilli()
            );
            return null;
        });
    }
