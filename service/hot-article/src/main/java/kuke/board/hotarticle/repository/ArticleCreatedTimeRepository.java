...
/**
 * 생성 시간을 관리하는 이유
 * 좋아요 이벤트가 있는데, 이 이벤트에 대한 게시글이 오늘 게시글인지 확인하려면, 게시글 서비스 조회가 필요함
 * 하지만 게시글 생성 시간을 저장하고 있으면, 오늘 게시글인지 게시글 서비스 안찔러봐도 바로 알 수 있음 !
 */
@Repository
...
public class ArticleCreatedTimeRepository {
    ...
    public void createOrUpdate(Long articleId, LocalDateTime createdAt, Duration ttl) {
        redisTemplate.opsForValue().set(
                ...
                String.valueOf(createdAt.toInstant(ZoneOffset.UTC).toEpochMilli()), ttl
        );
    }
