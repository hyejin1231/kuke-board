...
class CommentServiceTest {
    ...
    void deleteShouldMarkedDeletedIfHasChildren() {
        //given
        Long articleId = 1L;
        Long commentId = 2L;
    ...
    void deleteShouldDeleteChildOnlyIfNotDeletedParent() {
        ...
        verify(commentRepository, never()).delete(parentComment); // 부모 댓글은 삭제되지 않았음을 검증
    }
    ...
    void deleteShouldDeleteAllRecursivelyIfDeletedParent() {
        ...
        // then
        verify(commentRepository).delete(comment);
        verify(commentRepository).delete(parentComment); // 부모 댓글은 삭제되지 않았음을 검증
    }
